...
TowerTemp::TowerTemp() : _sensors_count(0),
                         oneWire{std::make_shared<OneWire>(ONE_WIRE_BUS)},
                         sensors{std::make_shared<DallasTemperature>(&*oneWire)} {}
...
void TowerTemp::begin()
{
    ...
    for (int i = 0; i < _sensors_count; i++)
    {
        ...
        {
            log_i("Found device index %d with address: %s", i, printAddress(temp_sensor_addresses, size).c_str(), DEC);
        }
        else
        {
